...
class UserController extends ControllerMVC {
  ...
  //     ),
  //   );
  //   Future<void>.delayed(const Duration(seconds: 1), () => _interstitialAd.load());
  //   Future<void>.delayed(const Duration(seconds: 3), () => _interstitialAd.show());
  //   ;
  // }
  ...
  //         }),
  //   );
  //   Future<void>.delayed(const Duration(seconds: 1), () => myRewarded.load());
  //   Future<void>.delayed(const Duration(seconds: 10), () => myRewarded.show());
  // }

  getUuId() async {
    final SharedPreferences pref = await SharedPreferences.getInstance();
    setState(() {
      iosUuId = pref.getString("ios_uuid") == null
          ? ""
          : pref.getString("ios_uuid").toString();
      iosEmail = pref.getString("ios_email") == null
          ? ""
          : pref.getString("ios_email").toString();
    });
          ...
          userRepo
              ...
              if (value) {
                ...
                if (userRepo.errorString.value != "") {
                  ...
                  ScaffoldMessenger.of(GlobalVariable.navState.currentContext!)
                      .showSnackBar(
                          SnackBar(content: Text(userRepo.errorString.value)));
                }
              }
            }
          }).catchError((e) {
            ...
            ScaffoldMessenger.of(GlobalVariable.navState.currentContext!)
                .showSnackBar(const SnackBar(
                    content: const Text('Sign In with Apple failed!')));
          });
        }
      } else {
        final Map<String, String> userInfo = {
    ...
    switch (fBResult.status) {
      case LoginStatus.success:
        ...
        userRepo.socialLogin(profile, timezone, 'FB').then((value) async {
          ...
          ScaffoldMessenger.of(GlobalVariable.navState.currentContext!)
              .showSnackBar(const SnackBar(
                  content: const Text("Facebook login failed!")));
        });
        break;
    ...
    if (googleSignIn.currentUser != null) {
      await getGoogleInfo(googleSignIn).then((profile) {
        userRepo.socialLogin(profile, timezone, 'G').then((value) {
          ...
          ScaffoldMessenger.of(GlobalVariable.navState.currentContext!)
              .showSnackBar(
                  const SnackBar(content: const Text("Google login failed!")));
        });
      });
    } else {
      Helper.hideLoader(loader);
  ...
  Future<void> followUnfollowUser(userId, index) async {
    ...
    userRepo.followUnfollowUser(userId).then((value) {
      ...
      ScaffoldMessenger.of(GlobalVariable.navState.currentContext!)
          .showSnackBar(
              const SnackBar(content: const Text("There are som error")));
    });
  }
  ...
  blockUser(userId) async {
    ...
    userRepo.blockUser(userId).then((value) async {
      ...
      if (response['status'] == 'success') {
        ...
        ScaffoldMessenger.of(GlobalVariable.navState.currentContext!)
            .showSnackBar(
                const SnackBar(content: const Text("There are some error")));
      }
    });
  }
  ...
  verifyOtp() async {
    ...
    if (response['status'] != 'success') {
      String msg = response['msg'];
      showAlertDialog(
          errorTitle: 'Error Verifying OTP',
          errorString: msg,
          fromLogin: false);
    } else {
      userRepo.setCurrentUser(value);
      ...
      userRepo.forgotPassword(data).then((value) async {
        ...
        if (response['status'] != 'success') {
          ...
          ScaffoldMessenger.of(GlobalVariable.navState.currentContext!)
              .showSnackBar(const SnackBar(
                  content: const Text(
                      "An OTP is sent to your email please check your email.")));
    ...
    videoRepo.deleteVideo(videoId).then((value) async {
      if (value != null) {
        ...
        if (response['status'] != 'success') {
          ...
          ScaffoldMessenger.of(GlobalVariable.navState.currentContext!)
              .showSnackBar(const SnackBar(
                  content: const Text("There's some error deleting video")));
        }
      }
    });
  }
